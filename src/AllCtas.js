...
const AllCtas = () => {
    ...
    const add=()=>{axios.post(SERVER, { title, price, cat }).then(res => setprods([prods, res.data]))}
    const del = (id) => {axios.delete(SERVER+ "/" +id).then(res => setrefFlag(!refFlag))}
    const upd = () => {axios.put(SERVER+ "/" +prods[0].id, { title, price, cat }).then(res => setprods([prods, res.data]))}
